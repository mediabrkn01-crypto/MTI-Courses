...
function renderLesson(lessonId){
  ...
  var LI={
    lock:'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="11" width="14" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>',
    ...
    left:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"/></svg>',
  ...
  app.innerHTML=sidebarHtml("")
    ...
    +'<div class="lesson-body-split" style="display:flex;flex:1;min-height:0;overflow:hidden">'
      +'<div class="lesson-left-col" style="flex:1;overflow-y:auto;min-width:0;padding-bottom:80px">'
        ...
        +'</div>'
        +'<div class="ls-body">'
          +'<div class="ls-eyebrow">Mission '+lesson.order+(videoMeta.duration?' · '+escapeHtml(videoMeta.duration):'')+'</div>'
          +'<h1 class="ls-title">'+videoTitle+'</h1>'
          +'<p class="ls-desc">'+videoDesc+'</p>'
          +'<div class="ls-grid">'
            +'<div class="ls-card">'
            ...
            +'<div class="ls-card ls-challenge">'
              +'<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:6px">'
                +'<div class="ls-card-h" style="margin:0">Mission challenge</div>'
                +(realQuiz?'<span class="ls-pill">7 questions</span>':'')
              +'</div>'
              +'<div id="ch-sub" style="font-size:13px;color:var(--muted2);margin-bottom:16px;line-height:1.5">'+(completed?'Lesson complete — test what you learned.':'Complete the mission video to unlock the challenge.')+'</div>'
              +(realQuiz?'<button type="button" id="ch-btn" onclick="navigate(\'quiz\',{lessonId:\''+lesson.id+'\'})" '+(completed?'':'disabled')
                ...
                +';display:flex;align-items:center;gap:8px;padding:12px 22px;border:none;border-radius:10px;color:#fff;font-family:Montserrat,sans-serif;font-weight:800;font-size:13px">⚡ '+(completed?'Start Challenge':'Unlock Challenge')+'</button>'
      ...
      +'<aside id="mp-right" class="mp-right-panel ls-side">'
        ...
        +'</div>'
        +'<div class="ls-list">'+missionList+'</div>'
      +'</aside>'
    +'</div>'
    +'</div>';
